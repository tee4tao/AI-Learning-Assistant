...
const QuizTakePage = () => {
  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      await quizService.submitQuizAnswers(quizId, formattedAnswers);
  ...
  return (
    <div className='max-w-4xl mx-auto'>
      ...
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl p-6 shadow-xl shadow-slate-200/50 mb-8">
        ...
        <div className="size-2 bg-emerald-500 rounded-ful animate-pulse" />
        ...
        <div className="space-y-3">
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <label key={index} className={`group relative flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all duration-200 ${isSelected ? 'border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-500/10' : 'border-slate-200 bg-slate-50/50 hover:border-slate-300 hover:bg-white hover:shadow-md'}`}>
                ...
                {/* Custom Radio Button */}
                <div className={`shrink-0 size-5 rounded-full border-2 transition-all duration-200 ${isSelected ? 'border-emerald-500 bg-emerald-500' : 'border-slate-300 bg-white group-hover:border-emerald-400'}`}>
                  {isSelected && (
                ...
                {/* Selected Checkmark */}
                {isSelected && (
                  <CheckCircle2 className="ml-auto size-5 text-emerald-500" strokeWidth={2.5} />
                )}
              </label>
            )
          })}
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {currentQuestionIndex === quiz.questions.length - 1 ? (
          <button
            ...
            disabled={submitting}
            className=" group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 text-white font-semibold text-sm rounded-xl hover:from-emerald-600 hover:to-teal-600 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden"
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              {submitting ? (
                <>
                <div className="size-4 border-2 border-white/30 border-t-white rounded-full animate-spin">Submitting...</div>
                </>
              ):(
                <>
      ...
      <div className="mt-0 flex items-center justify-center gap-2 flex-wrap">
        {quiz.questions.map((_, index) => {
          const isAnsweredQuestion = selectedAnswers.hasOwnProperty(quiz.questions[index].id);
